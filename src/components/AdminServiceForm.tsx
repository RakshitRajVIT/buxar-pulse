...
export default function AdminServiceForm({ onClose }: AdminServiceFormProps) {
	...
	const loadServices = async () => {
		...
		try {
			...
			console.error('Failed to load services list.', loadError);
		} finally {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			setFormData({
				name: '', address: '', category: '', phone: '', photoUrl: '',
				rating: '', reviewsCount: '', isOpen: true, lat: '', lng: '',
	...
	return (
		<form onSubmit={handleSubmit} className="space-y-4 mt-4">
			<div>
				...
				<input type="text" name="name" value={formData.name} onChange={handleChange} placeholder="e.g., Hotel Grand Palace"
			...
			<div className="grid grid-cols-2 gap-3">
				<div>
					...
					<select name="category" value={formData.category} onChange={handleChange}
						...
						{categories.map(cat => (
							<option key={cat.id} value={cat.id} style={{ color: 'black' }}>{cat.label}</option>
						))}
					</select>
				</div>
				<div>
					<label className="block text-xs font-semibold text-muted-foreground mb-1.5">Rating</label>
					<input type="number" name="rating" value={formData.rating} onChange={handleChange} step="0.1" placeholder="e.g., 4.5"
						className="w-full px-3 py-2 rounded-lg bg-foreground/5 border border-foreground/10 text-foreground text-sm focus:outline-none focus:border-foreground/30" />
				</div>
			</div>
			...
			<div>
				...
				<input type="text" name="address" value={formData.address} onChange={handleChange} placeholder="e.g., NH-84, Buxar"
					className="w-full px-3 py-2 rounded-lg bg-foreground/5 border border-foreground/10 text-foreground text-sm focus:outline-none focus:border-foreground/30" />
			</div>
			...
			<div className="grid grid-cols-2 gap-3">
				...
				<div>
					<label className="block text-xs font-semibold text-muted-foreground mb-1.5">Review Count</label>
					<input type="number" name="reviewsCount" value={formData.reviewsCount} onChange={handleChange} placeholder="e.g., 120"
						className="w-full px-3 py-2 rounded-lg bg-foreground/5 border border-foreground/10 text-foreground text-sm focus:outline-none focus:border-foreground/30" />
				</div>
			</div>
            {/* Lat & Lng Row */}
			...
			<div>
				...
				<input type="text" name="photoUrl" value={formData.photoUrl} onChange={handleChange} placeholder="https://example.com/image.jpg"
					className="w-full px-3 py-2 rounded-lg bg-foreground/5 border border-foreground/10 text-foreground text-sm focus:outline-none focus:border-foreground/30" />
			</div>

			<div>
				<label className="block text-xs font-semibold text-muted-foreground mb-1.5">Google Maps URL</label>
				<input type="text" name="googleMapsUrl" value={formData.googleMapsUrl} onChange={handleChange} placeholder="https://maps.google.com/?q=..."
					className="w-full px-3 py-2 rounded-lg bg-foreground/5 border border-foreground/10 text-foreground text-sm focus:outline-none focus:border-foreground/30" />
			</div>

			<div>
				<label className="block text-xs font-semibold text-muted-foreground mb-1.5">Description</label>
				<textarea name="description" value={formData.description} onChange={handleChange} placeholder="Brief summary..." rows={3}
			...
			<label className="flex items-center gap-2 cursor-pointer">
				<input type="checkbox" name="isOpen" checked={formData.isOpen} onChange={handleChange} className="w-4 h-4" />
			...
			{success && <div className="p-3 rounded-lg bg-green-500/10 border border-green-500/20 text-green-400 text-xs">{success}</div>}

			<button type="submit" disabled={isSubmitting}
				className="w-full h-10 rounded-xl text-sm font-semibold transition-all hover:scale-105 active:scale-95"
				style={{ background: 'linear-gradient(135deg, #4F46E5, #A78BFA)', color: 'white', opacity: isSubmitting ? 0.6 : 1 }}>
			...
			<div className="pt-5 mt-4 border-t border-foreground/10">
				<div className="flex items-center justify-between mb-3">
					...
					<button type="button" onClick={loadServices} className="text-xs text-muted-foreground hover:text-foreground">Refresh</button>
				</div>
				...
				{isLoadingList ? (
					...
					<div className="space-y-2 max-h-64 overflow-y-auto pr-1">
						{services.map((service) => (
							<div key={service.id} className="flex items-center justify-between gap-2 p-2 rounded-lg border border-foreground/10 bg-foreground/[0.03]">
								<div className="min-w-0">
									<p className="text-xs font-semibold text-foreground truncate">{service.name?.trim() || 'Unnamed Service'}</p>
									<p className="text-[11px] text-muted-foreground truncate">{service.category || 'no-category'}{service.address ? ` • ${service.address}` : ''}</p>
								</div>
								<button type="button" onClick={() => handleDeleteService(service.id)} disabled={isDeleting === service.id}
									className="px-2.5 py-1.5 rounded-md text-[11px] font-semibold"
									style={{ background: 'rgba(239,68,68,0.18)', color: '#ef4444', opacity: isDeleting === service.id ? 0.7 : 1 }}>
